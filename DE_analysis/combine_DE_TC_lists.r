...
deseq_gen_gene_suf <- '_DESeq2_TC_genes.txt'
masig_gen_gene_suf <- '_TC_DE_genes.txt'
...
combo_out_suf <- '_TC_DE_smRNAs_List2.txt'
...
both_hits <- intersect(deseq_gen_gene$gene, masig_gen_gene$genes)

combo_df <- data.frame(locus_name = both_hits, stringsAsFactors = F)

deseq_inds <- which(deseq_gen_gene$gene %in% combo_df$locus_name)
combo_df$deseq2_pval <- deseq_gen_gene$LRT_adj_pval[deseq_inds]

masig_inds <- which(masig_gen_gene$genes %in% combo_df$locus_name)
combo_df$masigpro_pval <- masig_gen_gene$p_val[masig_inds]
